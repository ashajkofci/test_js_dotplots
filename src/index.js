  ...
  async function initializePlot() {
    ...
    const token =
      ...
      try {
        ...
        plotScatterWithDensity(FL1, FL2, 'FL1 (log10)', 'FL2 (log10)', 'scatter-plot', 'Parula');
        ...
        document.getElementById('btn-fl1-ssc').addEventListener('click', () => {
            plotScatterWithDensity(FL1, SSC, 'FL1 (log10)', 'SSC (log10)', 'scatter-plot', 'Plasma');
        });

        document.getElementById('btn-ssc-fsc').addEventListener('click', () => {
            plotScatterWithDensity(SSC, FSC, 'SSC (log10)', 'FSC (log10)', 'scatter-plot', 'Inferno');
        });
...
function calculateDensity(x, y, bins, range) {
    const xEdges = Array.from({ length: bins + 1 }, (_, i) => range[0] + i * (range[1] - range[0]) / bins);
    const yEdges = Array.from({ length: bins + 1 }, (_, i) => range[0] + i * (range[1] - range[0]) / bins);

    const normalizedX = x.map(val => (val - range[0]) / (range[1] - range[0]) * bins);
    const normalizedY = y.map(val => (val - range[0]) / (range[1] - range[0]) * bins);
    ...
    return {
        density: x.map((_, i) => {
            const xIndex = Math.floor(normalizedX[i]);
            const yIndex = Math.floor(normalizedY[i]);
            return (xIndex >= 0 && xIndex < bins && yIndex >= 0 && yIndex < bins) ? histogram[xIndex][yIndex] / maxDensity : 0;
        }),
        histogram: histogram
    };
}
...
function plotScatterWithDensity(x, y, xLabel, yLabel, containerId, colormap) {
    ...
    const colorMaps = {
        Parula: ['#352a87', '#0363e1', '#1485d4', '#06a7c6', '#38b99e', '#92bf73', '#d9ba56', '#fcce2e', '#f9fb0e'],
    ...
    const filteredPoints = x.map((val, i) => ({ x: x[i], y: y[i], density: density[i] }))
        .filter(point => point.x >= 2 && point.x <= 6.7 && point.y >= 2 && point.y <= 6.7);
    ...
    const xHistogram = sortedX.reduce((acc, val) => {
        const bin = Math.floor((val - range[0]) / (range[1] - range[0]) * bins);
        acc[bin] = (acc[bin] || 0) + 1;
        return acc;
    }, Array(bins).fill(0));

    const yHistogram = sortedY.reduce((acc, val) => {
        const bin = Math.floor((val - range[0]) / (range[1] - range[0]) * bins);
        acc[bin] = (acc[bin] || 0) + 1;
        return acc;
    }, Array(bins).fill(0));
    ...
    const option = {
        ...
        grid: [
            ...
            { left: '10%', right: '25%', height: '15%', bottom: '10%' },  // Y-axis histogram grid
        ...
        series: [
            {
                type: 'scatter',
                data: sortedX.map((val, i) => [sortedX[i], sortedY[i], sortedDensity[i]]),
                ...
                itemStyle: {
                    color: function (params) {
                        ...
                        const colorIndex = Math.min(Math.floor(value * (colormapColors.length - 1)), colormapColors.length - 1);
            ...
            {
                type: 'line',
                data: [[4.2, 0], [4.2, 3.2], [6.7, 5.9], [6.7, 0.0]], // Example polygon points
                lineStyle: {
                    color: 'red',
                    width: 3
                },
